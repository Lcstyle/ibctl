...
pub trait AgentApi: Send + Sync {
    ...
    fn click_button(&self, window_id: u64, label: &str) -> impl std::future::Future<Output = Result<bool, AgentError>> + Send;
    fn type_text(&self, window_id: u64, field_index: usize, text: &str) -> impl std::future::Future<Output = Result<bool, AgentError>> + Send;
    fn click_menu(&self, window_id: u64, menu_path: &str) -> impl std::future::Future<Output = Result<bool, AgentError>> + Send;
    fn set_checkbox(&self, window_id: u64, label: &str, state: Option<bool>) -> impl std::future::Future<Output = Result<bool, AgentError>> + Send;
    fn select_list_item(&self, window_id: u64, item_text: &str) -> impl std::future::Future<Output = Result<bool, AgentError>> + Send;
...
impl AgentClient {
    ...
    pub async fn click_button(&self, window_id: u64, label: &str) -> Result<bool, AgentError> {
        self.inner.click_button_boxed(window_id, label).await
    }
    pub async fn type_text(&self, window_id: u64, field_index: usize, text: &str) -> Result<bool, AgentError> {
        self.inner.type_text_boxed(window_id, field_index, text).await
    }
    pub async fn click_menu(&self, window_id: u64, menu_path: &str) -> Result<bool, AgentError> {
        self.inner.click_menu_boxed(window_id, menu_path).await
    }
    pub async fn set_checkbox(&self, window_id: u64, label: &str, state: Option<bool>) -> Result<bool, AgentError> {
        self.inner.set_checkbox_boxed(window_id, label, state).await
    }
    pub async fn select_list_item(&self, window_id: u64, item_text: &str) -> Result<bool, AgentError> {
        self.inner.select_list_item_boxed(window_id, item_text).await
    }
    pub async fn click_at(&self, window_id: u64, x: i32, y: i32) -> Result<bool, AgentError> {
        self.inner.click_at_boxed(window_id, x, y).await
    }
    pub async fn select_tree_node(&self, window_id: u64, node_name: &str) -> Result<bool, AgentError> {
...
trait AgentApiBoxed: Send + Sync {
    ...
    fn click_button_boxed<'a>(&'a self, window_id: u64, label: &'a str) -> BoxFut<'a, Result<bool, AgentError>>;
    fn type_text_boxed<'a>(&'a self, window_id: u64, field_index: usize, text: &'a str) -> BoxFut<'a, Result<bool, AgentError>>;
    fn click_menu_boxed<'a>(&'a self, window_id: u64, menu_path: &'a str) -> BoxFut<'a, Result<bool, AgentError>>;
    fn set_checkbox_boxed<'a>(&'a self, window_id: u64, label: &'a str, state: Option<bool>) -> BoxFut<'a, Result<bool, AgentError>>;
    fn select_list_item_boxed<'a>(&'a self, window_id: u64, item_text: &'a str) -> BoxFut<'a, Result<bool, AgentError>>;
...
impl AgentApi for UdsAgent {
    ...
    async fn click_button(&self, window_id: u64, label: &str) -> Result<bool, AgentError> {
    ...
    async fn type_text(&self, window_id: u64, field_index: usize, text: &str) -> Result<bool, AgentError> {
        let path = format!("/windows/{}/type", window_id);
        let body = serde_json::json!({ "fieldIndex": field_index, "text": text });
        let resp: AgentResponse<serde_json::Value> = self.post(&path, &body).await?;
        Ok(resp.ok)
    }
    async fn click_menu(&self, window_id: u64, menu_path: &str) -> Result<bool, AgentError> {
        let path = format!("/windows/{}/menu", window_id);
        let body = serde_json::json!({ "path": menu_path });
        let resp: AgentResponse<serde_json::Value> = self.post(&path, &body).await?;
        Ok(resp.ok)
    }
    async fn set_checkbox(&self, window_id: u64, label: &str, state: Option<bool>) -> Result<bool, AgentError> {
    ...
    async fn select_list_item(&self, window_id: u64, item_text: &str) -> Result<bool, AgentError> {
        let path = format!("/windows/{}/selectlist", window_id);
        let body = serde_json::json!({ "item": item_text });
        let resp: AgentResponse<serde_json::Value> = self.post(&path, &body).await?;
        Ok(resp.ok)
    }
    async fn click_at(&self, window_id: u64, x: i32, y: i32) -> Result<bool, AgentError> {
        let path = format!("/windows/{}/clickat", window_id);
        let body = serde_json::json!({ "x": x.to_string(), "y": y.to_string() });
        let resp: AgentResponse<serde_json::Value> = self.post(&path, &body).await?;
        Ok(resp.ok)
    }
    async fn select_tree_node(&self, window_id: u64, node_name: &str) -> Result<bool, AgentError> {
    ...
    async fn send_key(&self, window_id: u64, key: &str) -> Result<bool, AgentError> {
...
impl AgentApi for MockAgent {
    ...
    async fn click_button(&self, _: u64, _: &str) -> Result<bool, AgentError> { Ok(self.click_result) }
    async fn type_text(&self, _: u64, _: usize, _: &str) -> Result<bool, AgentError> { Ok(true) }
    async fn click_menu(&self, _: u64, _: &str) -> Result<bool, AgentError> { Ok(self.click_result) }
    async fn set_checkbox(&self, _: u64, _: &str, _: Option<bool>) -> Result<bool, AgentError> { Ok(true) }
    async fn select_list_item(&self, _: u64, _: &str) -> Result<bool, AgentError> { Ok(true) }
...
mod tests {
    ...
    async fn test_mock_click_button() {
        ...
        assert!(!client.click_button(1, "OK").await.unwrap());
    }
}
